...
// Function pointers
void (*findLeaf[52])(int* leaves, float* features,
...
{0, &findLeafDepth1, &findLeafDepth2, &findLeafDepth3, &findLeafDepth4,
 ...
 &findLeafDepth49, &findLeafDepth50, &findLeafDepth51};
 /*&findLeafDepth52,
...
int main(int argc, char** args) {
  ...
  struct timeval start, end;
    int   remainder = nbTrees % T;

  gettimeofday(&start, NULL);
  //DOT (not consider remainig trees):
  ...
  for(iIndex = 0; iIndex < numberOfInstances; iIndex+=F) {
    for(tindex = 0; tindex < nbTrees; tindex+=T)
      for (i=0; i<F; i+=V)
        for(j=0; j<T; j++)
        {
          findLeaf[treeDepths[tindex+j]](leaf, &features[(iIndex+i) * numberOfFeatures],
                                   numberOfFeatures, &all_nodes[nodeSizes[tindex+j]]);
          for(k = 0; k < V; k++) {
            scores[k]+=all_nodes[nodeSizes[tindex+j]+leaf[k]].theta;
          }
        }
    for(k = 0; k < V; k++) {
      if(printScores) {
  ...
  // deal with remaining trees
    for(iIndex = 0; iIndex < numberOfInstances; iIndex+=F)
      for (j=0; j<remainder; j++)
        for(i=0; i<F; i+=V)
        {
          findLeaf[treeDepths[nbTrees - remainder +j]](leaf, &features[(iIndex+i) * numberOfFeatures],
                                   numberOfFeatures, &all_nodes[nodeSizes[nbTrees -remainder +j]]);
          for(k = 0; k < V; k++) {
            scores[k]+=all_nodes[nodeSizes[nbTrees - remainder +j]+leaf[k]].theta;
          }
        }
    for(k = 0; k < V; k++) {
      if(printScores) {
