...
void *BusyWork(void *t)
{
  ...
  divide = nbTrees / NUM_THREADS;
  begin_index = (int)tid * divide;
  end_index = (int)(tid+1) * divide;
  sum = 0;
  int remainder = end_index % T;
  for(tindex = begin_index; tindex < end_index -remainder; tindex+=T) {
    score = 0;
...
int main(int argc, char** args) {
  ...
  while(divisibleNumberOfInstances % (F*NUM_THREADS) !=0)
