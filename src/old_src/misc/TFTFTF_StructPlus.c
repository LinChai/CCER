...
int main(int argc, char** args) {
  ...
  divide_number = nbTrees / TT;
  remainder = nbTrees % TT;
 ...
 /* 
  ...
  //SOT:
    ...
    int jj, ii;
  for (tindex = 0; tindex < nbTrees; tindex+=TT) {
    score = 0;
    for (iIndex = 0; iIndex < numberOfInstances-FF; iIndex+=FF) 
        for (jj = 0; jj < TT; jj+=T)
            for (ii = 0; ii < FF; ii+=F)
              for (j=0; j<T; j++)
                for (i=0; i<F; i++) {
                  //printf("%d\t%d\n", tindex+jj+j, iIndex+ii+i);
                  score += getLeaf(trees[tindex+jj+j], features[iIndex+ii+i])->threshold;
                  }
    if (printScores) 
      printf("%f\n", score);
    sum += score;
